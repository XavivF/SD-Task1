...
class InsultFilterWorker:
    def __init__(self, worker_id: str, stop_event: Event):
        ...
        self.insults_cache = []  # Cache local opcional, però Redis és prou ràpid
    ...
    def _connect_rabbitmq(self):
        try:
            ...
            # Assegura que el exchange d'insults existeix per si aquest worker també volgués escoltar-lo
            self.channel.exchange_declare(exchange=config.INSULTS_EXCHANGE_NAME, exchange_type='fanout', durable=True)
            # Per consumir d'un en un i fer ack/nack manualment
            ...
            time.sleep(5)
            self._connect_rabbitmq()  # Retry

    def _filter_text(self, text: str) -> str:
        # Per simplicitat, obtenim insults de Redis cada cop.
        # Per optimitzar, es podria tenir un cache local i actualitzar-lo periòdicament
        # o quan es rep un senyal/missatge de nou insult.
        current_insults = redis_cli.get_all_insults()
        # print(f"[Worker {self.worker_id}] Current insults for filtering: {current_insults}")

        words = text.split()
        censored_words = []
        for word in words:
            # Treiem puntuació bàsica per a la comparació
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                method_frame, properties, body = self.channel.basic_get(queue=config.TEXT_QUEUE_NAME, auto_ack=False)
                if method_frame:
                    text_to_filter = body.decode('utf-8')
                    print(f"[Worker {self.worker_id}] Received text: '{text_to_filter}'")

                    censored_text = self._filter_text(text_to_filter)
                    print(f"[Worker {self.worker_id}] Censored text: '{censored_text}'")

                    redis_cli.add_censored_text(censored_text)
                    redis_cli.increment_processed_count()  # Incrementa el comptador global

                    self.channel.basic_ack(delivery_tag=method_frame.delivery_tag)
                else:
                    # No message, wait a bit to avoid busy-waiting if stop_event is not set
                    time.sleep(0.1)
...
if __name__ == '__main__':
    ...
    # Cal que RabbitMQ i Redis estiguin funcionant
    # Cal crear insults a Redis manualment per provar:
    # redis-cli -> SADD insults_set "beneit"
    # redis-cli -> SADD insults_set "capsigrany"

    # Enviar missatges a la cua 'text_queue' via RabbitMQ UI o un client pika
    # Exemple: publicat "Ets un beneit i un capsigrany"
